...
如图，Cookie在第一次访问时时没有的。  
![Cookie](./images/set-cookie.png "Cookie")  
#### session
虽然Cookie可以存储一些用户信息，但是Cookie是在本地存储的，如果有些敏感信息存储在客户端是不安全的。这样有些服务器为了解决这个问题，它在用户身份确认后，会将用户信息存储在服务器上，每次要获取用户信息是，通过一个类似ID的信息来与服务器端的信息匹配。这样就可以把敏感是数据存储在服务器上了，而网络上只会传输一个ID信息。通常把这种在服务器端保存回话状态的技术就叫Session。  
如上面Cookie中我们设置的JSESSIONID，然后每次代理访问服务器的时候，HTTP协议会携带Cookie，这是服务器端获取到Cookie信息的JSESSIONID，通过它就可以获取到存储到服务器端的用户信息了。
#### Web缓存
![Web缓存](./images/request_cache_01.png "Web缓存")  
图5  


![Web缓存](./images/response_cache_304_01.png "Web缓存")  
图6  
Web缓存是可以自动保存常见文档副本的HTTP设备。当Web请求到达缓存时，如果本地有“已缓存的”副本。就可以从本地存储设备而不是原始服务器去取文档。
HTTP协议中，可以通过GET请求的If-Modified-Since来请求验证服务器当前资源是否在指定的时间之前修改了，如果没有修改，则返回304 Not Modified。如果发生了修改，则返回一条普通的200 OK的响应。如果资源以及被删除，则返回404 Not Found响应。如图5，发起了一个带有If-Modifified-Since的请求。图6是返回的304的响应。


好，关于HTTP协议的内容大致就此，HTTP协议还有许多的细节，具体可以参考《HTTP权威指南》。
